...
import type { ScoringContext, ScoringResult } from '../types/Score.ts';
...
export function scoreHand(
  ...
  function scoreCard(card: PlayingCard, isRetrigger = false): void {
    ...
    // Base chips from card
    let cardChips = card.baseChips;
    if (card.enhancement === 'stone') cardChips = 50;
    emitStep(ctx, `${source}:chips`, { addChips: cardChips });

    // Enhancement chips
    if (card.enhancement === 'bonus') emitStep(ctx, `${source}:bonus`, { addChips: 30 });

    // Enhancement mult
    if (card.enhancement === 'mult') emitStep(ctx, `${source}:mult`, { addMult: 4 });

    // Glass: ×2 mult, 1/4 chance destroy
    if (card.enhancement === 'glass') {
      emitStep(ctx, `${source}:glass`, { mulMult: 2 });
    ...
    if (card.enhancement === 'lucky') {
      ...
      if (rng.nextBool(luckyProb / 5)) emitStep(ctx, `${source}:lucky_mult`, { addMult: 20 });
      if (rng.nextBool(luckyProb / 15)) deferredMoney.push(20);
    }

    // Edition chips
    if (card.edition === 'foil') emitStep(ctx, `${source}:foil`, { addChips: 50 });
    if (card.edition === 'holographic') emitStep(ctx, `${source}:holo`, { addMult: 10 });
    if (card.edition === 'polychrome') emitStep(ctx, `${source}:poly`, { mulMult: 1.5 });
    ...
    for (const joker of runState.jokers) {
      ...
      if (result.addChips) emitStep(ctx, `joker:${joker.instanceId}:chips`, { addChips: result.addChips });
      if (result.addMult) emitStep(ctx, `joker:${joker.instanceId}:mult`, { addMult: result.addMult });
      if (result.mulMult) emitStep(ctx, `joker:${joker.instanceId}:xmult`, { mulMult: result.mulMult });
  ...
  for (const card of heldCards) {
    ...
    if (card.enhancement === 'steel') {
      emitStep(ctx, `held:${card.id}:steel`, { mulMult: 1.5 });
    }
    if (card.seal === 'blue') {
  ...
  if (mime) {
    for (const card of heldCards) {
      if (card.isDebuffed) continue;
      if (card.enhancement === 'steel') {
        emitStep(ctx, `mime:${card.id}:steel`, { mulMult: 1.5 });
      }
    }
  }
  ...
  for (const joker of runState.jokers) {
    ...
    if (joker.edition === 'foil') emitStep(ctx, `joker:${joker.instanceId}:foil`, { addChips: 50 });
    if (joker.edition === 'holographic') emitStep(ctx, `joker:${joker.instanceId}:holo`, { addMult: 10 });
    if (joker.edition === 'polychrome') emitStep(ctx, `joker:${joker.instanceId}:poly`, { mulMult: 1.5 });
  }
